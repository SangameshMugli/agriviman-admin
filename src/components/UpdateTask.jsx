...
function UpdateTask() {
  ...
  useEffect(() => {
    // Fetch task details for updating
    axios
      .get(`http://localhost:5001/api/fetchtask/${service_id}`)
      .then((response) => {
        const taskData = response.data;
        console.log("Fetched task data:", taskData); // Debug: Check API response
        // Ensure date is in the correct format YYYY-MM-DD
        const formattedDate = taskData.assigned_date
          ? new Date(taskData.assigned_date).toISOString().split("T")[0]
          : "";
        setFormData({
          acres: taskData.acres || "",
          field_type: taskData.field_type || "",
          service: taskData.service || "",
          assigned_to_pilot_id: taskData.assigned_to_pilot_id || "",
          assigned_date: formattedDate, // Set formatted date
          status: taskData.status || "",
          contact_request_id: taskData.contact_request_id || "",
        });
      })
  ...
  useEffect(() => {
    const fetchPilotsAndRequests = async () => {
      try {
        ...
        const pilotsResponse = await axios.get(
          "http://localhost:5001/api/fetchpilot",
          {
            headers: {
        ...
        const contactRequestsResponse = await axios.get(
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      axios
        .put(`http://localhost:5001/api/updatetask/${service_id}`, formData)
        .then((response) => {
          ...
          <div className="w-full  bg-white p-4 mx-10 rounded-lg shadow-lg">
            <div className="w-full flex justify-between my-4">
              ...
              <button
                ...
                id={`update${formData.service_id}`}
              >
                <ReactTooltip
                  anchorId={`update${formData.service_id}`}
                  place="bottom"
                  content="Close"
                  variant="info"
                />
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex justify-center">
                <button
                  type="submit"
                  className="bg-light-green text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-green focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-75"
                >
                  Update Task
                </button>
              </div>
